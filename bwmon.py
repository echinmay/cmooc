...
class BandwidthMonitor(app_manager.RyuApp):
    ...
    def statsReplyHandler(self, ev):
        ...
        # [ ADD YOUR CODE HERE ]
        for stat in body:
            # Check if the name is of a edgeSwitch
            if name in self.topo.edgeSwitches.keys():
                # Check if the port number is not virtual
                if stat.port_no in self.topo.ports[name].keys():
                   # Get the peer depending on the port_no
		   peernode = self.topo.ports[name][stat.port_no]
                   # Check if the peer is a host
                   if peernode in self.topo.hosts.keys(): 
                      # Add stats
        	      self.bwstats.addHostBwStat(peernode, stat.tx_bytes, stat.rx_bytes)
